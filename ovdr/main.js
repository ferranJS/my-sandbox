...
function prepareCanvas() {
    ...
    c_out.setAttribute('width', containerWidth)
    c_out.setAttribute('height', containerHeight)

    c_tmp.setAttribute('width', containerWidth)
    c_tmp.setAttribute('height', containerHeight)

    c_nodes.setAttribute('width', containerWidth)
    c_nodes.setAttribute('height', containerHeight)
    
    video_in.style.width = containerWidth + 'px'
    video_in.style.height = containerHeight + 'px'
    video_out.style.width = containerWidth + 'px'
    video_out.style.height = containerHeight + 'px'
...
function computeFrame(once=false) {
    let cw = c_out.width
    let ch = c_out.height
    let vw = video_in.videoWidth
    let vh = video_in.videoHeight
    
    let scale = Math.min(cw / vw, ch / vh)
    let drawW = vw * scale
    let drawH = vh * scale
    let offsetX = (cw - drawW) / 2
    let offsetY = (ch - drawH) / 2
    
    ctx_out.clearRect(0, 0, cw, ch)
    ctx_out.drawImage(video_in, offsetX, offsetY, drawW, drawH)
    ctx_out.drawImage(c_tmp, 0, 0)
...
function getPosition(e) {
    ...
    let canvasWidth = c_tmp.width
    let canvasHeight = c_tmp.height
...
function next() {
    let nextTxt = document.getElementById("nextTxt")
    //canvas->video
